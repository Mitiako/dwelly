    ...
    <div className='space-y-6'>
      ...
      <div className='grid grid-cols-4 gap-4'>
        {selectedCities.map((cityId, index) => {
          ...
          return (
            <Card key={cityId} className='relative'>
              <button
  ...
  <X size={14} />
              ...
              <div className='mt-2 pt-2 border-t border-[#222222]'>
                <p className='text-gray-300 text-xs'>
                  {data.saleData.averageDaysOnMarket} days on market
                </p>
              </div>
            </Card>
          )
        ...
        {selectedCities.length < 4 && (
          <div className='relative'>
            <button
              ...
              className='w-full h-full min-h-[120px] border border-dashed border-[#333333] rounded-xl flex flex-col items-center justify-center gap-2 text-gray-500 hover:border-cyan-400 hover:text-cyan-400 transition-all'
            >
              <Plus size={20} />
      ...
      {/* Price Trends Chart */}
      <Card>
        <h2 className='text-white font-semibold mb-6'>Price Trends (12 Months)</h2>
        <MultiLineChart
          data={chartData}
      ...
      {/* Comparison Table */}
      <Card>
        <h2 className='text-white font-semibold mb-6'>Market Statistics</h2>
            ...
            <tbody className='divide-y divide-[#1a1a1a]'>
              {[
                ...
                },
                {
                  ...
                  getValue: (id: string) =>
                    formatNumber(mockMarketData[id]?.saleData.totalListings || 0),
                },
                {
                  label: 'New Listings',
                  getValue: (id: string) =>
                ...
                },
                {
                  label: 'Average Rent',
                  getValue: (id: string) =>
                    formatPrice(mockMarketData[id]?.rentalData.averageRent || 0),
                },
              ].map(row => (
                <tr key={row.label}>
                  <td className='text-gray-400 text-sm py-3 pr-4'>{row.label}</td>
